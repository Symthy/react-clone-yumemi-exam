...
import { usePopulationDatasetsState } from './usePopulationDatasetsState';

describe('usePopulationDatasetsState test', () => {
  test('success', () => {
    const inputData = [
      {
        prefName: '徳島県',
    ...
    const { result, rerender } = renderHook(() => usePopulationDatasetsState());
    result.current.savePopulationDatasets(inputData);
    rerender();

    expect(result.current.perfectureToPopulationDataSets).toStrictEqual(expected);
    expect(result.current.statisticsItems).toStrictEqual(['総人口', '生産年齢人口']);
  });
});
